...
exports.createOracleAPI = createOracleAPI;
...
async function callLLMProvider(provider, model, apiKey, prompt, maxTokens = 500, temperature = 0.1) {
...
function createOracleAPI() {
    ...
    router.post('/oracle/evaluate', async (req, res) => {
        try {
            const { prompt, provider, model, apiKey, maxTokens, temperature } = req.body;
            ...
            const rawResponse = await callLLMProvider(provider, model, apiKey, prompt, maxTokens, temperature);
    ...
    router.get('/oracle/health', (req, res) => {
        res.json({
            status: 'healthy',
    ...
    router.post('/oracle/consensus', async (req, res) => {
        try {
            ...
            for (const providerConfig of providers) {
                try {
                    const rawResponse = await callLLMProvider(providerConfig.provider, providerConfig.model, providerConfig.apiKey, prompt, maxTokens, temperature);
                    const oracleResponse = parseOracleResponse(rawResponse);
                    responses.push({
